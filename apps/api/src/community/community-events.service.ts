...
/**
 * Fans community connection changes out to SSE clients. The changes arrive from
 * PostgreSQL `NOTIFY`, so a probe run by any API replica — a live feed's
 * signal, a Re-check, a snapshot write-back — reaches every open page.
 *
 * Each stream also carries the feed status: which platforms are pushing their
 * changes right now. It is sent when the client subscribes and again whenever a
 * feed changes state, so an open page can say what it is hearing from rather
 * than assuming it is hearing anything.
 */
@Injectable()
export class CommunityEventsService implements OnModuleInit, OnModuleDestroy {
   ...
   * later one is a change, a feed status update, or a heartbeat.
   */
