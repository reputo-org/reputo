...
// The wire form is hyphenated (`deep-id`) so it stays stable across HTTP and
// Temporal. The Prisma enum label cannot contain hyphens, so the schema maps
// `deep_id @map("deep-id")` and the generated TS literal is `'deep_id'`.
// Translate at the repository boundary so callers above the repos keep using
// the wire form.
