...
describe('proposal_engagement (e2e)', () => {
  ...
  it('rewards funded-concluded and penalizes unfunded proposals per weights', async () => {
    ...
    // The CSV carries the raw scores (alice 1.8 = 2 × 0.9, bob −0.4 = −1 × 0.4) — no
    // normalization, so a negative score stays negative.
    expect(rows).toEqual([
      { did: 'did:plc:alice', proposal_engagement: '1.8' },
      { did: 'did:plc:bob', proposal_engagement: '-0.4' },
    ]);
  });
...
describe('proposal_engagement (e2e) — edge cases', () => {
  ...
  it('credits a scored proposal to both its proposer and team members', async () => {
    ...
    const rows = parseCsv(storage.readText(outputs.proposal_engagement as string) as string);
    // Both owners are credited the same raw reward (1) — the proposal is not split.
    expect(rows).toEqual([
      { did: 'did:plc:alice', proposal_engagement: '1' },
      { did: 'did:plc:bob', proposal_engagement: '1' },
    ]);
  ...
  it('applies monthly decay to the proposal reward', async () => {
    ...
    const rows = parseCsv(storage.readText(outputs.proposal_engagement as string) as string);
    // Raw score 0.8 = tw 0.8 × norm 1 (both asserted from the details below).
    expect(rows).toEqual([{ did: 'did:plc:alice', proposal_engagement: '0.8' }]);
